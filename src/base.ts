...
export const createBaseResult = () => ({
  ...
  iter: {
     ...
     * const mapped = iter.map(error, x => x * 2);
     ...
     * const enhanced = await iter.mapAsync(userPromise, async (user) => {
     ...
     * const enhanced = iter.mapErr(result, error => ({ code: 404, message: error }));
     ...
     * @example
     * ```typescript
     * const result = await iter.mapErrAsync(apiCall(), async (error) => {
     ...
     * @example
     * ```typescript
     * const result = iter.andThen(getUser(1), user =>
     *   iter.andThen(getProfile(user.id), profile =>
     *     ok({ user, profile })
     *   )
     * );
     * ```
     ...
     * @example
     * ```typescript
     * const result = await iter.andThenAsync(
     *   fetchUser(1),
  ...
  batch: {
     ...
     * const combined = batch.all(results);
     * // Returns: Ok([1, 2, 3])
     *
     * const mixed = [ok(1), err("failed"), ok(3)];
     * const failed = batch.all(mixed);
     * // Returns: Err("failed")
     * ```
     *
    ...
    /**
     * Waits for all Promise<Result> to settle, then combines like batch.all().
     *
     * @example
     * ```typescript
     * const promises = [fetchUser(1), fetchUser(2), fetchUser(3)];
     * const result = await batch.allAsync(promises);
     ...
     * ```typescript
     * const promises = [fetchUser(1), fetchUser(2), fetchUser(3)];
     * const { oks, errors } = await batch.allSettledAsync(promises);
     ...
     * const successes = batch.oks(results);
     * // Returns: [1, 3]
     * ```
     *
     ...
     * ```typescript
     * const results = [ok(1), err("failed"), ok(3)];
     * const stats = batch.partitionWith(results);
     ...
     * const { firstOk, firstError, okIndex, errorIndex } = batch.findFirst(results);
     ...
     * const sum = batch.reduce(results, {
     ...
     * const first = batch.first(results);
     * // Returns: Ok("success")
     *
     * const allFailed = [err("e1"), err("e2")];
     * const none = batch.first(allFailed);
     * // Returns: Err(["e1", "e2"])
     * ```
     *
     * @param results - Array of Results to search
  ...
  advanced: {
     ...
     * @example
     * ```typescript
     * const result = await advanced.safeAsync(async function* () {
     ...
     * ```typescript
     * const result = advanced.safe(function* () {
     *   const user = yield* advanced.yieldFn(getUser(id));
     *   return user;
     * });
     * ```
     *
     ...
     * const combined = advanced.zip(nameResult, emailResult);
     ...
     * const value = ok(5);
     * const result = advanced.apply(addFn, value);
     ...
     * const result = advanced.apply(
     *   advanced.apply(add, parseNumber("5")),
     *   parseNumber("3")
     * );
     * // Returns: Ok(8)
     * ```
  ...
  utils: {
     ...
     * const result = iter.pipe(
     *   fetchUser(id),
     *   user => utils.tap(user, u => console.log("Loaded:", u.name)),
     ...
     * const result = utils.fromNullable(user, "User not found");
     * // Returns: Ok(user) or Err("User not found")
     *
     * // With custom error:
     * const result2 = utils.fromNullable(user, { code: 404, message: "Not found" });
     * ```
     *
     ...
     * const user = utils.toNullable(result);
