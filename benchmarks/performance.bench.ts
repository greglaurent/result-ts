...
import { err, isErr, isOk, ok } from "../dist/core.js";
import type { Err, Ok, Result } from "../dist/types.js";
...
interface User {
	...
	profile?: {
		bio: string;
		avatar?: string;
		// biome-ignore lint/suspicious/noExplicitAny: Realistic user preferences can be any type
...
describe("Single-Pass vs Multiple-Pass Operations", () => {
	...
	bench("❌ naive multiple passes", () => {
		const _total = largeResults.length;
		const _okCount = largeResults.filter(isOk).length;
		const _errorCount = largeResults.filter(isErr).length;
		const _hasErrors = _errorCount > 0;
		const _isEmpty = _total === 0;
	});
	...
	bench("❌ functional equivalent to partitionWith", () => {
		...
		const _oks = okResults.map((r) => r.value);
		const _errors = errResults.map((r) => r.error);
		const _okCount = _oks.length;
		const _errorCount = _errors.length;
		const _total = largeResults.length;
	});
});
...
describe("Zero-Allocation Loops vs Functional Chains", () => {
	...
	bench("❌ naive filter twice", () => {
		const _oks = mediumResults.filter(isOk).map((r) => r.value);
		const _errors = mediumResults.filter(isErr).map((r) => r.error);
	});
});
...
describe("User Data Processing (Real-World)", () => {
	bench("✅ Process user validation results", () => {
		...
		if (stats.hasErrors) {
			...
			const _errorsByField = errors.reduce(
				(acc, error) => {
		...
		const _adultUsers = validUsers.filter((user) => user.age >= 18);
	});
	...
	bench("✅ Complex user data extraction", () => {
		const { oks: users, errors: _errors } = partition(mediumUserResults);
		const _usersWithProfiles = users.filter((u) => u.profile);
		const _avgAge = users.reduce((sum, u) => sum + u.age, 0) / users.length;
		const _emailDomains = new Set(users.map((u) => u.email.split("@")[1]));
	});

	bench("❌ Naive complex extraction", () => {
		const users = mediumUserResults.filter(isOk).map((r) => r.value);
		const _errors = mediumUserResults.filter(isErr).map((r) => r.error);
		const _usersWithProfiles = users.filter((u) => u.profile);
		const _avgAge = users.reduce((sum, u) => sum + u.age, 0) / users.length;
		const _emailDomains = new Set(users.map((u) => u.email.split("@")[1]));
	});
...
describe("API Response Processing (Real-World)", () => {
	bench("✅ Process API batch responses", () => {
		...
		const _totalUsers = successfulResponses.reduce(
			(sum, response) => sum + response.data.length,
			0,
		);
		const errors = errs(apiResults);
		const _serverErrors = errors.filter((e) => e.status >= 500);
	});
...
describe("File Processing Pipeline (Real-World)", () => {
	bench("✅ File processing with batch operations", () => {
		...
		const _largeFiles = files.filter((file) => file.size > 500);
		const _errorsByType = errors.reduce(
			(acc, error) => {
...
describe("Async Operations (Real-World)", () => {
	...
	bench("❌ Promise.all with manual error handling", async () => {
		...
		const _successes = results
			.filter(({ result }) => isOk(result))
			.map(({ result }) => (result as Ok<unknown>).value);
		const _errors = results
			.filter(({ result }) => isErr(result))
			.map(({ result }) => (result as Err<unknown>).error);
	});
});
...
describe("Cross-Module Integration (Real-World)", () => {
	bench("✅ Full validation + processing pipeline", () => {
		...
		const _enhancedUsers = validUsers.map((user) => ({
			...user,
		...
		// 3. Analyze results
		const _stats = analyze(results);

		// 4. Error categorization
		const _errorCategories = validationErrors.reduce(
			(acc, error) => {
	...
	bench("❌ Manual pipeline without result-ts", () => {
		...
		const _okCount = validUsers.length;
		const errorCount = validationErrors.length;
		const _hasErrors = errorCount > 0;
...
describe("Memory Pressure (Large Objects)", () => {
	...
	bench("❌ manual analysis with large objects", () => {
		const _total = largeObjectResults.length;
		const _okCount = largeObjectResults.filter(isOk).length;
		const _errorCount = largeObjectResults.filter(isErr).length;
	});
});
...
describe("Edge Cases Performance", () => {
	const createEdgeCaseResults = () => {
		...
		for (let i = 0; i < 1000; i++) {
			if (i % 5 === 0) {
				// biome-ignore lint/suspicious/noExplicitAny: Testing null handling, requires any
...
describe("Scaling Performance (Real-World Sizes)", () => {
	...
	bench("Small dataset (100 items) - full pipeline", () => {
		...
		if (stats.hasErrors) {
			const { oks: users, errors: _errors } = partition(smallUserResults);
			const _avgAge = users.reduce((sum, u) => sum + u.age, 0) / users.length;
		}
	});
	...
	bench("Large dataset (10,000 items) - full pipeline", () => {
		...
		if (stats.hasErrors) {
			const { oks: users, errors: _errors } = partition(largeUserResults);
			const _avgAge = users.reduce((sum, u) => sum + u.age, 0) / users.length;
		}
	});
});
